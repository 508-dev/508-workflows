...
- `/create-mailbox`
  ...
  - Brevo newsletter sync: if `BREVO_API_KEY` is configured, the new 508 mailbox and backup email are added to `BREVO_508_MEMBERS_NEWSLETTER_LIST_ID`, or to the Brevo list named by `BREVO_508_MEMBERS_NEWSLETTER_LIST_NAME` (default `508 members`) when the explicit ID is unset. Brevo failures are shown as warnings and do not block mailbox creation.
  - Required role: Admin
  - Args:
