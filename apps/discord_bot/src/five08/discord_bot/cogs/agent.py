...
class AgentConfirmationView(discord.ui.View):
    ...
    async def cancel(
        ...
        await interaction.response.defer(ephemeral=True)
        await self.cog._post_agent_confirmation(
            plan_id=self.plan_id,
            context=self.context,
            confirm=False,
        )
        self._disable()
        await interaction.followup.send("Agent plan canceled.", ephemeral=True)
        if interaction.message is not None:
            try:
                await interaction.message.edit(view=self)
            except discord.HTTPException:
...
class AgentCog(DiscordAuditCogMixin, commands.Cog):
    ...
    async def agent_command(
        ...
        self._audit_command_safe(
            interaction=interaction,
            action="agent.request",
            result="success" if response.get("status") != "failed" else "error",
            metadata={
