...
_DEFINITIONS: tuple[RuntimeConfigDefinition, ...] = (
    ...
    RuntimeConfigDefinition(
        key="ERPNEXT_BANK_TRANSACTION_WEBHOOK_SIGNING_SECRET",
        attr="erpnext_bank_transaction_webhook_signing_secret",
        label="ERPNext Bank Transaction webhook secret",
        category="Projects",
        description="Frappe HMAC secret accepted only for submitted Bank Transaction webhook hints.",
        is_secret=True,
        env_names=("ERPNEXT_BANK_TRANSACTION_WEBHOOK_SIGNING_SECRET",),
    ),
    RuntimeConfigDefinition(
        key="PROJECT_PAYMENT_AUTOMATION_ENABLED",
        attr="project_payment_automation_enabled",
        label="Project payment automation enabled",
        category="Projects",
        description="Allow safe automatic project-payment routing actions to execute.",
        value_type="bool",
        env_names=("PROJECT_PAYMENT_AUTOMATION_ENABLED",),
        restart_required=True,
    ),
    RuntimeConfigDefinition(
        key="PROJECT_PAYMENT_NOTIFICATIONS_ENABLED",
        attr="project_payment_notifications_enabled",
        label="Project payment Discord notifications enabled",
        category="Projects",
        description="Allow the worker to deliver confirmed project-payment receipts through the Discord bot.",
        value_type="bool",
        env_names=("PROJECT_PAYMENT_NOTIFICATIONS_ENABLED",),
    ),
    RuntimeConfigDefinition(
        key="PROJECT_PAYMENT_RECOVERY_INTERVAL_SECONDS",
        attr="project_payment_recovery_interval_seconds",
        label="Project payment recovery interval seconds",
        category="Projects",
        description="Seconds between recovery sweeps for stalled payment actions, learned suggestions, and notification deliveries.",
        value_type="int",
        env_names=("PROJECT_PAYMENT_RECOVERY_INTERVAL_SECONDS",),
        restart_required=True,
        min_value=60,
    ),
    RuntimeConfigDefinition(
