...
_DEFINITIONS: tuple[RuntimeConfigDefinition, ...] = (
    ...
    RuntimeConfigDefinition(
        key="ONBOARDING_REMINDERS_ENABLED",
        attr="onboarding_reminders_enabled",
        label="Onboarding reminders enabled",
        category="Onboarding",
        description="Whether the Discord bot sends overdue onboarding reminders.",
        value_type="bool",
        env_names=("ONBOARDING_REMINDERS_ENABLED",),
        restart_required=True,
    ),
    RuntimeConfigDefinition(
        key="ONBOARDING_REMINDER_STALE_DAYS",
        attr="onboarding_reminder_stale_days",
        label="Onboarding reminder stale days",
        category="Onboarding",
        description="Days without onboarding activity before the first reminder.",
        value_type="int",
        env_names=("ONBOARDING_REMINDER_STALE_DAYS",),
        min_value=1,
    ),
    RuntimeConfigDefinition(
        key="ONBOARDING_REMINDER_REPEAT_DAYS",
        attr="onboarding_reminder_repeat_days",
        label="Onboarding reminder repeat days",
        category="Onboarding",
        description="Days between reminders while a candidate remains inactive.",
        value_type="int",
        env_names=("ONBOARDING_REMINDER_REPEAT_DAYS",),
        min_value=1,
    ),
    RuntimeConfigDefinition(
        key="ONBOARDING_REMINDER_CHECK_SECONDS",
        attr="onboarding_reminder_check_seconds",
        label="Onboarding reminder check interval",
        category="Onboarding",
        description="Seconds between checks for overdue onboarding reminders.",
        value_type="int",
        env_names=("ONBOARDING_REMINDER_CHECK_SECONDS",),
        restart_required=True,
        min_value=60,
    ),
    RuntimeConfigDefinition(
        key="DISCORD_ONBOARDING_VOLUNTEERS_CHANNEL_ID",
        attr="discord_onboarding_volunteers_channel_id",
        label="Onboarding volunteers channel ID",
        category="Onboarding",
        description="Discord channel receiving weekly reminders for selected candidates.",
        env_names=("DISCORD_ONBOARDING_VOLUNTEERS_CHANNEL_ID",),
        restart_required=True,
    ),
    RuntimeConfigDefinition(
    ...
    RuntimeConfigDefinition(
        key="JOB_LEAD_JEV_SHADOW_MAX_CALLS",
        attr="job_lead_jev_shadow_max_calls",
        label="Jev job lead shadow maximum calls",
        category="AI",
        description="Maximum Jev requests allowed during one scrape run.",
        value_type="int",
        env_names=("JOB_LEAD_JEV_SHADOW_MAX_CALLS",),
        min_value=1,
        max_value=100,
    ),
    RuntimeConfigDefinition(
        key="JOB_LEAD_JEV_SHADOW_RUN_BUDGET_SECONDS",
        attr="job_lead_jev_shadow_run_budget_seconds",
        label="Jev job lead shadow run budget seconds",
        category="AI",
        description="Total wall-clock budget for Jev requests during one scrape run.",
        value_type="float",
        env_names=("JOB_LEAD_JEV_SHADOW_RUN_BUDGET_SECONDS",),
        min_value=0.1,
        max_value=60.0,
    ),
    RuntimeConfigDefinition(
