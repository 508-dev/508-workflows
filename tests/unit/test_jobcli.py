...
def test_jobsctl_status_defaults_to_web_service_in_docker(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("API_SHARED_SECRET", "from-env")
    monkeypatch.delenv("WORKER_API_BASE_URL", raising=False)
    ...
    with (
        ...
        mock_request.return_value = _json_response(
            ...
            payload={"status": "succeeded", "job_id": "job-123"},
            method="GET",
            url="http://web:8090/jobs/job-123",
        )
    ...
    assert exit_code == 0
    mock_request.assert_called_once()
    assert mock_request.call_args.kwargs["url"] == "http://web:8090/jobs/job-123"
